...
void trackInventory();
void printStockReport(); // Function to print stock report

// for User
void addItemToCart();
void viewCart(); 
void buyStock();
void checkoutCart(const string& username); // New: Checkout cart function
...
void staffDashboard() { // just add add for allowing staff actions
    ...
    do {
        ...
        cout << "[5] Checkout Cart" << endl; // New: Checkout Cart option
        cout << "[6] Logout" << endl;
        cout << "Enter your choice: ";
        cin >> choice;
...
void buyStock() {
    ...
    if (it != stocks.end()) {
        if (it->getQuantity() >= quantity) {
            ...
            int receiptId = ExcelUtil::getNextReceiptId(receipts); // Get next available receipt ID
